...
const useForm = (initialState, validationRules) => {
  ...
  const validateField = useCallback((name, value, allValues) => {
    let error = validationRules[name] ? validationRules[name](value, allValues) : null;
    ...
    if (name === 'confirm_password' || name === 'password') {
      ...
      const confirmPasswordValue = name === 'confirm_password' ? value : allValues.confirm_password;
    ...
    setValues(prevValues => {
      ...
      setErrors(prevErrors => ({
        ...
        ...(name === 'password' || name === 'confirm_password' 
          ? { confirm_password: validateField('confirm_password', newValues.confirm_password, newValues) }
          : {})
      }));
      return newValues;
    });
